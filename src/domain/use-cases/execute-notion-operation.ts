...
import { toolTypes, type NotionAuthType, type ToolType } from '../value-types/tool';
    ...
    >
{
      ...
      case 'date':
        ...
        if (value.range === true) {
          result = {
            ...
            date: {
              start: moment
                .tz(value.dateStart as number, timezoneValue as string)
                .format(format),
              end: moment
                .tz(value.dateEnd as number, timezoneValue as string)
                .format(format),
            },
          };
        } else {
          result = {
            ...
            date: {
              start: moment
                .tz(value.date as number, timezoneValue as string)
                .format(format),
              end: null,
            },
          };
  ...
  export function mapFilters(filtersList: IDataObject[], timezone: string) {
    return filtersList.reduce((obj, value: { [key: string]: any }) => {
      ...
      if (key === 'rich_text' || key === 'text') {
        ...
        valuePropertyName =
          value.date === '' ? {} : moment.tz(value.date, timezone).utc().format();
      } else if (key === 'boolean') {
        key = 'checkbox';
      }
    ...
    if (['text'].includes(property.type as string)) {
      ...
      if (Array.isArray(property[type]) && property[type].length !== 0) {
        result = property[type]
          .map((text: any) => simplifyProperty(text) as string)
          .join('');
      } else {
        result = '';
      }
    ...
    for (const {
      ...
      if (
        ...
        results.push({
          id,
          ...(version === 2
            ? { name: getPropertyTitle(properties as IDataObject) }
            : {}),
          ...(version === 2 ? { url } : {}),
          ...(version === 2
            ? {
                ...prepend(
                  'property',
                  simplifyProperties(properties) as IDataObject
                ),
              }
        ...
        results.push({
          json: {
            id: json.id,
            ...(version === 2
              ? { name: getPropertyTitle(json.properties as IDataObject) }
              : {}),
            ...
            ...(version === 2
              ? {
                  ...prepend(
                    'property',
                    simplifyProperties(json.properties) as IDataObject
                  ),
                }
  ...
  export function getConditions() {
    ...
    elements.push({
      ...
      options: Object.keys(formula).map((key: string) => ({
    ...
    for (const key of Object.keys(formula)) {
      elements.push({
        ...
        options: formula[key].map((entry: string) => ({
          name: capitalCase(entry),
          value: entry,
        })),
        default: '',
