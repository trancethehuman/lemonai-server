...
const CODE_LANGUAGES = ['javaScript', 'json', 'python'] as const;
const CODE_EXECUTION_MODES = [
  'runOnceForAllItems',
  'runOnceForEachItem',
] as const;
...
export type ExecutionError = unknown;
...
export interface IAuthenticateBase {
  type: string;
  properties: Record<string, string> | IRequestOptionsSimplifiedAuth;
}
...
// The encrypted credentials which the nodes can access
export type ICredentialDataDecryptedObject = Record<
  string,
  CredentialInformation
>;
...
export type IGetExecuteFunctions = (
  workflow: unknown,
...
export type IGetExecuteSingleFunctions = (
  workflow: unknown,
  runExecutionData: IRunExecutionData,
  runIndex: number,
  connectionInputData: INodeExecutionData[],
...
export type IGetExecuteHookFunctions = (
  workflow: unknown,
  node: INode,
  additionalData: IWorkflowExecuteAdditionalData,
  mode: WorkflowExecuteMode,
...
export type IGetExecuteWebhookFunctions = (
  workflow: unknown,
  node: INode,
  additionalData: IWorkflowExecuteAdditionalData,
  mode: WorkflowExecuteMode,
...
export type ISourceDataConnections = Record<
  string,
...
export type IContextObject = Record<string, any>;

export type IExecuteContextData = Record<string, IContextObject>;
...
namespace ExecuteFunctions {
  ...
  export interface GetNodeParameterFn {
...
interface BaseHelperFunctions {
  createDeferredPromise: () => Promise<unknown>;
}
...
export interface RequestHelperFunctions {
  request: (
    uriOrObject: string | IDataObject | any,
    options?: IDataObject
  ) => Promise<any>;
...
type FunctionsBaseWithRequiredKeys<Keys extends keyof FunctionsBase> =
  FunctionsBase & {
...
export interface IPollFunctions
  ...
  __emitError: (error: Error, responsePromise?: unknown) => void;
  getNodeParameter: (
    parameterName: string,
    fallbackValue?: any,
    options?: IGetNodeParameterOptions
...
export interface ITriggerFunctions
  ...
  emitError: (error: Error, responsePromise?: unknown) => void;
  getNodeParameter: (
    parameterName: string,
    fallbackValue?: any,
    options?: IGetNodeParameterOptions
...
export type IBinaryKeyData = Record<string, IBinaryData>;
...
export interface IDisplayOptions {
  hide?: Record<string, NodeParameterValue[] | undefined>;
  show?: Record<string, NodeParameterValue[] | undefined>;
}
...
export type INodePropertyValueExtractorFunction = (
...
export type IParameterDependencies = Record<string, string[]>;

export interface IParameterLabel {
  size?: 'small' | 'medium';
}
...
export type INodeIssueObjectProperty = Record<string, string[]>;
...
export type IWorkflowIssues = Record<string, INodeIssues>;
...
export interface KnownNodesAndCredentials {
...
export type ITaskDataConnections = Record<
  string,
  Array<INodeExecutionData[] | null>
>;

// Keeps data while workflow gets executed and allows when provided to restart execution
export type IWaitingForExecution = Record<
  string,
  Record<number, ITaskDataConnections>
>;

export type ITaskDataConnectionsSource = Record<
  string,
  Array<ISourceData | null>
>;

export type IWaitingForExecutionSource = Record<
  string,
  Record<number, ITaskDataConnectionsSource>
>;
...
export type IWorkflowCredentials = Record<
  string,
  Record<string, ICredentialsEncrypted>
>;
...
export interface IWorkflowExecuteHooks {
  ...
  workflowExecuteBefore?: Array<
    (workflow: unknown, data: IRunExecutionData) => Promise<void>
  >;
  sendResponse?: Array<
    (response: IExecuteResponsePromiseData) => Promise<void>
  >;
}
...
export interface IWorkflowExecuteAdditionalData {
  ...
  httpRequest?: unknown;
  restApiUrl: string;
  setExecutionStatus?: (status: unknown) => void;
...
export type IStatusCodeMessages = Record<string, string>;
...
export type AllEntities<M> = M extends Record<string, string>
  ? Entity<M, keyof M>
  : never;
...
export type PropertiesOf<M extends { resource: string; operation: string }> =
  Array<
...
export type INodesGraphNode = Record<string, INodeGraphItem>;

export type INotesGraphNode = Record<string, INoteGraphItem>;
...
export type INodeNameIndex = Record<string, string>;
...
export type FeatureFlags = Record<string, string | boolean | undefined>;
...
export interface PublicInstalledNode {
  name: string;
  type: string;
...
export interface IExecutionsSummary {
  ...
  nodeExecutionStatus?: Record<string, IExceutionSummaryNodeExecutionResult>;
}
...
export interface IN8nUISettings {
  ...
  versionCli: string;
  n8nMetadata?: Record<string, string | number | undefined>;
